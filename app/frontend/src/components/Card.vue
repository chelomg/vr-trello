...
export default {
  ...
  methods: {
    ...
    linkify (inputText) {
      ...
      let text = inputText.replace(pattern1, '<a href="$1" class="text-blue-500 underline" target="_blank">link</a>')

      const pattern2 = /(^|[^/])(www\.[\S]+(\b|$))/gim
      text = text.replace(pattern2, '$1<a href="http://$2" class="text-blue-500 underline" target="_blank">link</a>')
      return text
    },
    DeleteCard (card) {
